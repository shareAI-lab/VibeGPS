...
- 每页 slide 容器基础尺寸为 100vw × 100vh。当某页信息量较大时，允许该页内容区域纵向滚动（overflow-y: auto），确保信息清晰完整展示，不要为了适配一屏而省略细节或减少信息量。由你自主判断哪些页面需要滚动、哪些可以一屏呈现
...
- **排版**: CSS Grid / Flexbox 布局，大字号、高对比度、充足留白。每页 slide 容器基础尺寸 \`width: 100vw; min-height: 100vh;\`，内容少时居中展示，内容多时允许纵向滚动（\`overflow-y: auto\`）。信息完整性优先于一屏适配。SVG 图形使用 \`viewBox\` 配合百分比宽高自适应。
